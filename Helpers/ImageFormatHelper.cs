...
namespace ExtractCLUT.Helpers
{
    public static class ImageFormatHelper
    {
        ...
        public static Image CreateScreenImage(List<SixLabors.ImageSharp.Image<Rgba32>> _tiles, ushort[] _mapData, int widthInTiles, int heightInTiles, int tileWidth, int tileHeight)
        {
            var tempScreenBitmap = new Bitmap(widthInTiles * tileWidth, heightInTiles * tileHeight);
            ...
            for (int y = 0; y < heightInTiles * tileHeight; y++)
            {
                for (int x = 0; x < widthInTiles * tileWidth; x++)
                {
                    ...
                    {
                        var pixel = tile[tilePixelX, tilePixelY]; // Accessing pixel using ImageSharp's 2D indexer
                        var color = Color.FromArgb(pixel.A, pixel.R, pixel.G, pixel.B); // Convert to System.Drawing.Color
