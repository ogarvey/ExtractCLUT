...
// var unk1 = unkReader.ReadUInt16();
// var count = unkReader.ReadUInt16();
// var unk2 = unkReader.ReadUInt16();
// var lineHeaderOffsets = new List<ushort>();

// for (int i = 0; i < count; i++)
// {
// 	lineHeaderOffsets.Add(unkReader.ReadUInt16());
// }

// Console.WriteLine($"Unk Size: {unkSize:X8}, Unk1: {unk1:X4}, Count: {count}, Unk2: {unk2:X4}");
// Console.WriteLine($"CurrentPos: {unkReader.BaseStream.Position:X8}");

// var lineInfos = new List<(ushort Offset, ushort Count, ushort Unk)>();

// for (int i = 0; i < count; i++)
// {
// 	unkReader.BaseStream.Seek(lineHeaderOffsets[i], SeekOrigin.Begin);
// 	var nextOffset = (i + 1 < count) ? lineHeaderOffsets[i + 1] : lineInfos[0].Offset;
// 	while (unkReader.BaseStream.Position + 6 < nextOffset)
// 	{
// 		var lineCount = unkReader.ReadUInt16();
// 		var lineUnk = unkReader.ReadUInt16();
// 		var lineOffset = unkReader.ReadUInt16();
// 		lineInfos.Add((lineOffset, lineCount, lineUnk));
// 		Console.WriteLine($"  Line Offset: {lineOffset:X4}, Count: {lineCount}, Unk: {lineUnk:X4}");
// 	}
// }

// Console.WriteLine($"CurrentPos: {unkReader.BaseStream.Position:X8}");

// var total = lineInfos.Sum(l => l.Count + l.Unk);
// Console.WriteLine($"Total data size: {total:X8}");
// var outputData = new List<byte>();
// foreach (var line in lineInfos)
// {
// 	unkReader.BaseStream.Seek(line.Offset, SeekOrigin.Begin);
// 	var lineData = unkReader.ReadBytes(line.Count);
// 	outputData.AddRange(lineData);
// 	outputData.AddRange(new byte[line.Unk]);
// }

// var outputFileUnk = Path.ChangeExtension(unkFile, ".decompressed.bin");
// File.WriteAllBytes(outputFileUnk, outputData.ToArray());

// var resource = @"C:\Dev\Gaming\PC\Dos\Games\Veil Of Darkness (1993)(Strategic Simulations Inc)\rooms";
// var outputDirResource = Path.Combine(Path.GetDirectoryName(resource)!, "rooms_output");
// Directory.CreateDirectory(outputDirResource);

// using var resourceReader = new BinaryReader(File.OpenRead(resource));
// var countResources = resourceReader.ReadUInt32();
// var offsetsResources = new List<uint>();
// for (int i = 0; i < countResources; i++)
// {
// 	offsetsResources.Add(resourceReader.ReadUInt32());
// }

// for (int i = 0; i < countResources; i++)
// {
// 	resourceReader.BaseStream.Seek(offsetsResources[i], SeekOrigin.Begin);
// 	var nextOffset = (i + 1 < countResources) ? offsetsResources[i + 1] : (uint)resourceReader.BaseStream.Length;
// 	var size = nextOffset - offsetsResources[i];
...
// var chaFiles = Directory.GetFiles(mainDir, "*.CHA", SearchOption.TopDirectoryOnly);
// var pidFiles = Directory.GetFiles(mainDir, "*.PID", SearchOption.TopDirectoryOnly);

// foreach (var chaFile in chaFiles)
// {
// 	var pidFile = Path.ChangeExtension(chaFile, ".PID");
// 	if (!File.Exists(pidFile))
// 	{
// 		Console.WriteLine($"Warning: No corresponding PID file found for CHA file: {Path.GetFileName(chaFile)}");
// 	}
// 	var palFile = Path.ChangeExtension(chaFile, ".PAL");
// 	if (!File.Exists(palFile))
// 	{
// 		Console.WriteLine($"Warning: No corresponding PAL file found for CHA file: {Path.GetFileName(chaFile)}");
// 		continue;
// 	}
...
// 	Console.WriteLine($"Loading PAL file: {palFile}");

// 	var pal1 = File.ReadAllBytes(palFile);

// 	// Extract sprites
// 	Console.WriteLine("\nExtracting sprites...");
// 	var sprites = CHASpriteDecompressor.ExtractAllSprites(chaData, pidData);
// 	Console.WriteLine($"  Found {sprites.Length} sprites");
// 	var outputDir = Path.Combine(Path.GetDirectoryName(chaFile)!, $"{Path.GetFileNameWithoutExtension(chaFile)}_sprites_output");
// 	Directory.CreateDirectory(outputDir);

// 	for (int i = 0; i < pal1.Length / 0x30; i++)
// 	{
// 		var palette = ColorHelper.ConvertBytesToRgbIS(pal1.Skip(i * 0x30).Take(0x30).ToArray(), true);
// 		var pal1Folder = Path.Combine(outputDir, $"palette_{i + 1}");
// 		Directory.CreateDirectory(pal1Folder);

// 		for (int s = 0; s < sprites.Length; s++)
// 		{
// 			var sprite = sprites[s];
// 			var outputData = ConvertPlanarToLinear(sprite.Pixels, sprite.Height, sprite.Width);
// 			/////////////File.WriteAllBytes(Path.Combine(outputDir, $"{s:D4}.raw"), outputData);

// 			var image = ImageFormatHelper.GenerateIMClutImage(palette, outputData, sprite.Height, sprite.Width, true);
// 			var outputPng = Path.Combine(pal1Folder, $"{s:D4}.png");
// 			image.Mutate(x => x.Rotate(90)); // correct orientation
...
// 	var (frameW, frameH) = AnimationExporter.ExportAnimationFrames(anim, animFramesDir, $"anim{i:D3}", 0, palette);
...
// var inputFileDir = @"C:\Dev\Gaming\PC\Dos\Eye_of_Typhoon_The_1996\eyeotyp\DATA\MAINDATA\DALMA_sprites_output";
// var inputFiles = Directory.GetFiles(inputFileDir, "*.raw", SearchOption.TopDirectoryOnly);
// var routputDir = Path.Combine(inputFileDir, "reordered_output");
// Directory.CreateDirectory(routputDir);
// int blockCount = 4;
// foreach (var inputFile in inputFiles)
// {
// 	var inputData = File.ReadAllBytes(inputFile);
// 	var outputData = ConvertPlanarToLinear(inputData, 96,96);
// 	// var sizeToUse = inputData.Length / blockCount;
// 	// var blockList = new List<byte[]>();


// 	// for (int i = 0; i < blockCount; i++)
// 	// {
// 	// 	var block = new byte[sizeToUse];
// 	// 	Buffer.BlockCopy(inputData, i * sizeToUse, block, 0, sizeToUse);
// 	// 	blockList.Add(block);
// 	// }

// 	// var outputData = new List<byte>();

// 	// while (outputData.Count < inputData.Length)
// 	// {
// 	// 	// add a byte from each block in order, then repeat
// 	// 	for (int i = 0; i < blockList.Count; i++)
// 	// 	{
// 	// 		var block = blockList[i];
// 	// 		var byteIndex = outputData.Count / blockList.Count;
// 	// 		if (byteIndex < block.Length)
// 	// 		{
// 	// 			outputData.Add(block[byteIndex]);
// 	// 		}
// 	// 	}
// 	// }
...
// {
// 	using var matoReader = new BinaryReader(File.OpenRead(matoFile));
// 	var outputDir = Path.Combine(Path.GetDirectoryName(matoFile)!, $"{Path.GetFileNameWithoutExtension(matoFile)}_output");
// 	Directory.CreateDirectory(outputDir);

// 	var magic = Encoding.ASCII.GetString(matoReader.ReadBytes(4));
// 	if (magic != "mato")
// 		continue;

// 	matoReader.ReadBytes(4); // file length
// 	var entryCount = matoReader.ReadBigEndianInt32();
// 	matoReader.ReadBytes(4); // unk
// 	var offsets = new List<uint>();
// 	for (int i = 0; i < entryCount; i++)
// 	{
// 		offsets.Add(matoReader.ReadBigEndianUInt32());
// 	}
// 	offsets.Add((uint)matoReader.BaseStream.Length);

// 	for (int i = 0; i < entryCount; i++)
// 	{
// 		matoReader.BaseStream.Seek(offsets[i], SeekOrigin.Begin);
// 		var actualOffset = matoReader.ReadBigEndianUInt32();
// 		if (actualOffset == 0)
// 			continue;
// 		var actualLength = matoReader.ReadBigEndianUInt32();
// 	}
// }

// var tdoDir = @"C:\Dev\Gaming\3do\Games\Crash 'n Burn\CNB\CarBitmaps";
// var tdoFiles = Directory.GetFiles(tdoDir, "*.3do", SearchOption.TopDirectoryOnly);

// foreach (var tdoFile in tdoFiles)
// {
// 	var outputDir = Path.Combine(Path.GetDirectoryName(tdoFile)!, $"{Path.GetFileNameWithoutExtension(tdoFile)}_output");
// 	Directory.CreateDirectory(outputDir);
// 	var imageIndex = 0;
// 	using var tdoReader = new BinaryReader(File.OpenRead(tdoFile));
// 	while (tdoReader.BaseStream.Position < tdoReader.BaseStream.Length)
// 	{
// 		Console.WriteLine($"Processing image {imageIndex} in file {Path.GetFileName(tdoFile)} at position {tdoReader.BaseStream.Position:X8}");
// 		var plutChunk = tdoReader.ReadBytes(0x4c);
// 		var pdatMagic = Encoding.ASCII.GetString(tdoReader.ReadBytes(4));
// 		if (pdatMagic != "PDAT")
// 			break;
// 		var palette = ColorHelper.ReadRgb15PaletteIS(plutChunk.Skip(0xC).ToArray());
// 		var pdatLength = tdoReader.ReadBigEndianInt32();
// 		tdoReader.BaseStream.Seek(-8, SeekOrigin.Current);
// 		var pdatData = tdoReader.ReadBytes(pdatLength);
// 		var outputData = CelUnpacker.UnpackCodedPackedWithDimensions(pdatData, 128, 128, 8, skipUncompSize: true);
// 		var outputPng = Path.Combine(outputDir, $"{imageIndex:D4}.png");
// 		CelUnpacker.SaveCelImage(outputData, outputPng, palette);
// 		imageIndex++;
// 	}
// }
...
// var framesListFile = @"C:\Dev\Gaming\3do\Games\SHADOW Warriors\ALVIN.FRAMES";
// var imageFolder = @"C:\Dev\Gaming\3do\Extractions\Shadow\Anvil (Alvin)";
// FrameListParser.ProcessMovesWithAlignment(framesListFile, moveListFile, imageFolder);
...
// var aOutputDir = Path.Combine(Path.GetDirectoryName(actFileTest)!, $"{Path.GetFileNameWithoutExtension(actFileTest)}_output");
// actFile.ExportImages(aOutputDir);

// var folderToResize = @"C:\Dev\Gaming\3do\Games\Bishoujo Senshi Sailor Moon S\data\ANIM\jupiter0_0\B04_output";
...
// var subFileDir = @"C:\Dev\Gaming\3do\Games\Bishoujo Senshi Sailor Moon S\data\Stage";
...
// var celDir = @"C:\Dev\Gaming\3do\Games\Blue-Forest-Story-Kaze-no-Fuin\bfd\battle_bg";
// var celFiles = Directory.GetFiles(celDir, "mpb1f.cel", SearchOption.AllDirectories);
...
// 		var magicBytes = File.ReadAllBytes(celFile).Take(4).ToArray();
// 		var magic = Encoding.ASCII.GetString(magicBytes);
// 		switch (magic)
// 		{
// 			case "CCB ":
// 				Directory.CreateDirectory(outputDir);
// 				CelUnpacker.UnpackAndSaveCelFile(celFile, outputPng, verbose: true);
// 				break;
// 			case "ANIM":
// 				Directory.CreateDirectory(outputDir);
// 				CelUnpacker.UnpackAnimFile(celFile, outputDir, verbose: false, noLoopRecords: true);
// 				break;
// 			case "IMAG":
// 				Directory.CreateDirectory(outputDir);
...
// var inputDir = @"C:\Dev\Gaming\PC\Win\Games\DKEEP_V1_0";
// var pcxFiles = Directory.GetFiles(inputDir, "*.PCX", SearchOption.AllDirectories);
// var pczFiles = Directory.GetFiles(inputDir, "*.PCZ", SearchOption.AllDirectories);

// foreach (var pcxFile in pcxFiles)
// {
// 	var outputPng = Path.ChangeExtension(pcxFile, ".png");
// 	var pcxData = File.ReadAllBytes(pcxFile);
// 	var pcxImage = ImageFormatHelper.ConvertPCX(pcxData);
// 	pcxImage.Save(outputPng);
// 	pcxImage = ImageFormatHelper.ConvertPCX(pcxData, true);
// 	outputPng = Path.ChangeExtension(pcxFile, ".transparent.png");
// 	pcxImage.Save(outputPng);
// }

// foreach (var pczFile in pczFiles)
// {
// 	var outputDir = Path.Combine(Path.GetDirectoryName(pczFile)!, $"{Path.GetFileNameWithoutExtension(pczFile)}_extracted");
// 	Directory.CreateDirectory(outputDir);
// 	using var pczReader = new BinaryReader(File.OpenRead(pczFile));
// 	var count = pczReader.ReadUInt16();

// 	for (int i = 0; i < count; i++)
// 	{
// 		if (pczReader.BaseStream.Position >= pczReader.BaseStream.Length)
// 			break;
// 		var pcxSize = pczReader.ReadUInt32();
// 		var pcxData = pczReader.ReadBytes((int)pcxSize);
// 		var pcxImage = ImageFormatHelper.ConvertPCX(pcxData, true);
...
// var dkDataDir = @"C:\Dev\Gaming\3do\Games\Deathkeep\data2\levels";
// var dkDataFiles = Directory.GetFiles(dkDataDir, "*data", SearchOption.TopDirectoryOnly);

// foreach (var dkDataFile in dkDataFiles)
// {
// 	using var dkReader = new BinaryReader(File.OpenRead(dkDataFile));
// 	var outputDir = Path.Combine(Path.GetDirectoryName(dkDataFile)!, "NEW", $"{Path.GetFileNameWithoutExtension(dkDataFile)}_extracted");
// 	Directory.CreateDirectory(outputDir);

// 	var count = dkReader.ReadBigEndianUInt32();

// 	for (int i = 0; i < count; i++)
// 	{
// 		var id = dkReader.ReadBigEndianUInt32();
// 		var size = dkReader.ReadBigEndianUInt32();
// 		if (id < 0x20)
// 		{
// 			dkReader.ReadBytes(8); // WHY??
// 		}
// 		var data = dkReader.ReadBytes((int)size);
// 		// var outputFile = Path.Combine(outputDir, $"{id:D4}.bin");
// 		// File.WriteAllBytes(outputFile, data);
// 	}

// 	while (dkReader.BaseStream.Position < dkReader.BaseStream.Length)
// 	{
// 		var dkMagic = Encoding.ASCII.GetString(dkReader.ReadBytes(4));
// 		while (dkMagic != "ANIM" && dkMagic != "CCB " && dkReader.BaseStream.Position < dkReader.BaseStream.Length)
// 		{
// 			dkMagic = Encoding.ASCII.GetString(dkReader.ReadBytes(4));
// 		}
// 		switch (dkMagic)
// 		{
// 			case "ANIM":
// 				{
// 					dkReader.BaseStream.Seek(-8, SeekOrigin.Current);
// 					var animLength = dkReader.ReadBigEndianUInt32();
// 					var animStartPos = dkReader.BaseStream.Position;
// 					var animData = dkReader.ReadBytes((int)animLength);
// 					var outputAnimFile = Path.Combine(outputDir, $"{animStartPos:X8}.anim");
// 					File.WriteAllBytes(outputAnimFile, animData);
// 					Console.WriteLine($"Extracted ANIM file: {outputAnimFile}");
// 				}
// 				break;
// 			case "CCB ":
// 				{
// 					dkReader.BaseStream.Seek(-8, SeekOrigin.Current);
// 					var celLength = dkReader.ReadBigEndianUInt32();
// 					var celStartPos = dkReader.BaseStream.Position;
// 					var celData = dkReader.ReadBytes((int)celLength);
// 					var outputCelFile = Path.Combine(outputDir, $"{celStartPos:X8}.cel");
// 					File.WriteAllBytes(outputCelFile, celData);
// 					Console.WriteLine($"Extracted CCB file: {outputCelFile}");
// 				}
// 				break;
// 		}
// 	}
...
// }
// var frameData = testData.Skip(0x860).Take(0x258).ToArray();
// var decompressedTest = FormatHelper.DecompressAnxType3(frameData, 0xF50, ushortParamTable);
// File.WriteAllBytes(Path.ChangeExtension(testBin, ".3_decompressedType3"), decompressedTest);
...
// var anxDir = @"C:\Dev\Gaming\PC\Dos\Games\Mario's Time Machine Deluxe\SURF";
// var anxFiles = Directory.GetFiles(anxDir, "*.ANX", SearchOption.TopDirectoryOnly);
// foreach (var anxFile in anxFiles)
// {

// 	using var anxReader = new BinaryReader(File.OpenRead(anxFile));
...
// 	var outputDir = Path.Combine(Path.GetDirectoryName(anxFile)!, $"{Path.GetFileNameWithoutExtension(anxFile)}_extracted");
// 	Directory.CreateDirectory(outputDir);
// 	for (int i = 0; i < fileCount; i++)
// 	{
...
// var testAnim = @"C:\Dev\Gaming\3do\Games\Deathkeep\data2\levels\NEW\10data_extracted\0006E228.anim";
// var testAnimOutputDir = Path.Combine(Path.GetDirectoryName(testAnim)!, $"{Path.GetFileNameWithoutExtension(testAnim)}_output");
// CelUnpacker.UnpackAnimFile(testAnim, testAnimOutputDir);

// var celDir = @"C:\Dev\Gaming\3do\Games\Deathkeep\data2\levels\NEW";
// var celFiles = Directory.GetFiles(celDir, "*.anim", SearchOption.AllDirectories);
// foreach (var celFile in celFiles)
// {
// 	var outputDir = Path.Combine(Path.GetDirectoryName(celFile)!, $"{Path.GetFileNameWithoutExtension(celFile)}_anim_output");
// 	CelUnpacker.UnpackAnimFile(celFile, outputDir);
// }


// var joinFile = @"C:\Dev\Gaming\3do\Games\Killing Time - Beta\Stream\ZStream";
// using var joinReader = new BinaryReader(File.OpenRead(joinFile));
// var joinOutputDir = Path.Combine(Path.GetDirectoryName(joinFile)!, "join_output_new_v2");
...
// var partFiles = new List<(string PartTag, bool isHdr, uint overallLength, int PartIndex, int partGroup, byte[] Data)>();
...
// 				var overallLength = joinReader.ReadBigEndianUInt32();
// 				if (blockType.Contains("HDR"))
// 				{
// 					joinReader.ReadBytes(0x8);
// 				}
// 				var partLength = blockType.Contains("HDR") ? joinReader.ReadBigEndianInt32() : 0;
// 				var blockData = joinReader.ReadBytes(blockLength - (blockType.Contains("HDR") ? 0x28 : 0x1c));
// 				if (blockType.Contains("HDR") && overallLength == blockLength - 0x28)
// 				{
...
// 					var outputFolder = Path.Combine(joinOutputDir, $"{blockType.Trim()}_whole");
...
// 				else
// 				{
// 					partFiles.Add((blockType.Trim(), blockType.Contains("HDR"), overallLength, blockFrame, blockGroup, blockData));
// 				}
...
// var headerParts = partFiles.Where(p => p.isHdr).ToList();

// foreach (var hdr in headerParts)
// {
// 	var matchingParts = partFiles.Where(p => !p.isHdr && p.partGroup == hdr.partGroup).OrderBy(p => p.PartIndex).ToList();
// 	Console.WriteLine($"\nProcessing HDR Part: {hdr.PartTag} Group: {hdr.partGroup} with {matchingParts.Count} matching parts.");
// 	Console.WriteLine($"Overall Length: {hdr.overallLength}, Total Parts Length: {matchingParts.Sum(p => (uint)p.Data.Length) + hdr.Data.Length}");
// 	var mergedData = new List<byte>();
// 	mergedData.AddRange(hdr.Data);
// 	while (mergedData.Count < hdr.overallLength)
// 	{
// 		foreach (var part in matchingParts)
// 		{
// 			mergedData.AddRange(part.Data);
// 			if (mergedData.Count >= hdr.overallLength)
// 				break;
// 		}
// 	}
// 	Console.WriteLine($"Merged Data Length: {mergedData.Count}, Expected Length: {hdr.overallLength}");
// 	var outputFileName = $"{hdr.partGroup}_merged.bin";
// 	var outputFilePath = Path.Combine(joinOutputDir, $"{hdr.PartTag}", outputFileName);
// 	Directory.CreateDirectory(Path.GetDirectoryName(outputFilePath)!);
// 	File.WriteAllBytes(outputFilePath, mergedData.ToArray());
// }
// Console.WriteLine($"Total parts collected: {partFiles.Count}, total Files: {partFiles.Count(f => f.isHdr)}.");
...
// foreach (var animFile in animFiles)
// {
// 	var outputDir = Path.Combine(Path.GetDirectoryName(animFile)!, Path.GetFileNameWithoutExtension(animFile) + "_output");
// 	try
//   {
// 		CelUnpacker.UnpackAnimFile(animFile, outputDir);
// 	} catch (Exception ex)
// 	{
// 		Console.WriteLine($"Error processing {Path.GetFileName(animFile)}: {ex.Message}");
// 	}
// }

var dataFileDir = @"C:\Dev\Gaming\3do\Games\Escape from Monster Manor";

// foreach (var dataFile in Directory.GetFiles(dataFileDir, "*.cels", SearchOption.TopDirectoryOnly))
// {
// 	var dataOutputDir = Path.Combine(Path.GetDirectoryName(dataFile)!, Path.GetFileNameWithoutExtension(dataFile) + "_output");
// 	Directory.CreateDirectory(dataOutputDir);
// 	using var dataReader = new BinaryReader(File.OpenRead(dataFile));
// 	//dataReader.BaseStream.Seek(0xC, SeekOrigin.Begin);
// 	var magic = Encoding.ASCII.GetString(dataReader.ReadBytes(4));
// 	while (magic != "CCB " && dataReader.BaseStream.Position < dataReader.BaseStream.Length)
// 	{
// 		magic = Encoding.ASCII.GetString(dataReader.ReadBytes(4));
// 	}
// 	if (dataReader.BaseStream.Position >= dataReader.BaseStream.Length)
// 		break;
// 	dataReader.BaseStream.Seek(-4, SeekOrigin.Current);
// 	while (dataReader.BaseStream.Position < dataReader.BaseStream.Length)
// 	{
// 		var currentPos = dataReader.BaseStream.Position;
// 		var ccbHeaderBlock = dataReader.ReadBytes(0x50);
// 		var plutHeader = Encoding.ASCII.GetString(dataReader.ReadBytes(0x4));
// 		var plutLength = 0;
// 		if (plutHeader == "PLUT")
// 		{
// 			plutLength = dataReader.ReadBigEndianInt32();
// 			dataReader.ReadBytes(plutLength - 0x8);
// 		}
// 		else
// 		{
// 			// No PLUT, rewind 4 bytes
// 			dataReader.BaseStream.Seek(-4, SeekOrigin.Current);
// 		}
// 		var xtraLength = 0;
// 		if (plutHeader == "XTRA")
...
// 			dataReader.BaseStream.Seek(-4, SeekOrigin.Current); 
// 			 xtraLength = 16;
// 			dataReader.ReadBytes(xtraLength);
// 		}
// 		var pdatHeader = dataReader.ReadBytes(0x4);
// 		var pdatLength = dataReader.ReadBigEndianInt32();
// 		var length = plutLength + xtraLength + pdatLength + 0x50;
// 		dataReader.BaseStream.Seek(currentPos, SeekOrigin.Begin);

// 		var celFile = dataReader.ReadBytes(length);
// 		var outputCel = Path.Combine(dataOutputDir, $"data_{dataReader.BaseStream.Position - length:X8}.cel");
// 		File.WriteAllBytes(outputCel, celFile);
// 		var outputPng = Path.ChangeExtension(outputCel, ".png");
// 		CelUnpacker.UnpackAndSaveCelFile(outputCel, outputPng);
// 		magic = Encoding.ASCII.GetString(dataReader.ReadBytes(4));
// 		while (magic == "PDAT")
// 		{
// 			var index = 1;
// 			var pdatLength2 = dataReader.ReadBigEndianInt32();
// 			dataReader.BaseStream.Seek(-8, SeekOrigin.Current);
// 			var pdatData = dataReader.ReadBytes(pdatLength2);
// 			// replace the celFile PDAT data with this one
// 			var pdatOffset = 0x50 + plutLength + xtraLength;
// 			var newCelFile = new byte[celFile.Length - pdatLength + pdatData.Length];
// 			Array.Copy(celFile, 0, newCelFile, 0, pdatOffset);
// 			Array.Copy(pdatData, 0, newCelFile, pdatOffset, pdatData.Length);
// 			outputCel = Path.Combine(dataOutputDir, $"data_{dataReader.BaseStream.Position - pdatLength2:X8}_alt{index++}.cel");
// 			File.WriteAllBytes(outputCel, newCelFile);
// 			outputPng = Path.ChangeExtension(outputCel, ".png");
// 			CelUnpacker.UnpackAndSaveCelFile(outputCel, outputPng);
// 			magic = Encoding.ASCII.GetString(dataReader.ReadBytes(4));
// 		}
// 		while (magic != "CCB " && dataReader.BaseStream.Position < dataReader.BaseStream.Length)
...
// var unkCelFilesDir = @"C:\Dev\Gaming\3do\Games\Deathkeep\data2";
// var unkCelFiles = Directory.GetFiles(unkCelFilesDir, "3DO.cel", SearchOption.AllDirectories);
// foreach (var unkCelFile in unkCelFiles)
// {
